    ...
    if (rank % 2 == 0) {
        if (rank < procs_count - 1) {
            ...
            for (j = 0; j != 11; ++j) {
                ...
                for (i = 0; i != 100; ++i) {
                    ...
                    // MPI_Issend(sendbuf, send_size, MPI_INT, rank + 1, 10, MPI_COMM_WORLD, &request);
                    // MPI_Recv(recvbuf, send_size + 100, MPI_INT, rank + 1, 20, MPI_COMM_WORLD, &status);
        ...
        for (i = 0; i != 11; ++i) {
            for (j = 0; j != 100; ++j) {
                ...
                // MPI_Issend(sendbuf, send_size, MPI_INT, rank - 1, 20, MPI_COMM_WORLD, &request);
                // MPI_Recv(recvbuf, send_size + 100, MPI_INT, rank - 1, 10, MPI_COMM_WORLD, &st);
                // MPI_Wait(&request, &st);
            }            
            send_size *= 2;
        }
