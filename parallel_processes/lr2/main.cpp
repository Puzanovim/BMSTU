    ...
    if (rank % 2 == 0) {
        if (rank < procs_count - 1) {
            ...
            for (j = 0; j != 11; ++j) {
                time = MPI_Wtime();
                for (i = 0; i != 100; ++i) {
                    MPI_Send(buf, send_size, MPI_INT, rank + 1, 10, MPI_COMM_WORLD);
                    MPI_Recv(buf, send_size + 100, MPI_INT, rank + 1, 20, MPI_COMM_WORLD, &status);
                }
                time = MPI_Wtime() - time;
        ...
        for (i = 0; i != 11; ++i) {
            for (j = 0; j != 100; ++j) {
                MPI_Recv(buf, send_size + 100, MPI_INT, rank - 1, 10, MPI_COMM_WORLD, &status);
                MPI_Send(buf, send_size, MPI_INT, rank - 1, 20, MPI_COMM_WORLD);
            }
            send_size *= 2;
        }
    }
